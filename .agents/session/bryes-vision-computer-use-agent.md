...
### [2026-07-17 11.32] (agent: software-architect) — `type_into` BATCHED TEXT-ENTRY ACTION (a DEVICE CAPABILITY, not loop weight) + OVERVIEW DOWNSCALE 0.5→0.375 🤖⌨️⚡

**Session theme**: Designed + shipped the recurring macro/combo backlog item as **`type_into`** — ONE Brain action performing the whole *[click? → clear? → type → Enter?]* text-entry gesture — via `/quick-wizard` → `/analyze-code-quality` → live FIT. A 3-turn naming/architecture co-design first (**names are load-bearing**), where Alvi's *"make it a hand capability on its own, not loop weight"* frame-kill made the design cleaner. Plus a quick `OVERVIEW_SCALE 0.5→0.375` tune, validated live.

**What happened**:
- **Design (locked over 3 turns)**: collapsed my 4 proposed named combos into ONE parameterized action (covers all 5, incl. the free click→clear→type→Enter *replace-and-submit* / WhatsApp-send pattern). Names are load-bearing (the `visual_focus` lesson): `type_into` + `click_target` / `clear_first` / `type_text` / `press_enter_after` (mechanism-honest — it presses Enter, it is NOT a button-submit). **Alvi's frame-kill**: I first put the composition in the LOOP; he pushed *"this should stand as a hand capability on its own, like `device.clear_field()`."* I conceded with **specific** reasons (symmetry with clear_field; the SUBMIT step is body-specific — a phone = send-button tap, not Enter; a thinner loop), holding the ONE boundary that **grounding stays loop-side** (the device has no Eyes → the loop grounds `click_target` → passes the pixel).
- **Load-bearing constraint (ef253360)**: ≤1 click, and it must be FIRST — grounding is against THIS step's frame, no mid-combo re-screenshot. Explicitly did NOT build a general multi-click sequence (YAGNI + stale-frame grounding).
- **Built (QW, 6 steps)**: `device.clear_field()` + `device.type_into()` on the Device Protocol + a shared `default_type_into` gesture (DRY — both bodies delegate; per-body `clear_field` + `act` key-mapping carry the differences); ContainerDevice (ctrl+a/Delete clear), PhoneDevice (delegates; `clear_field` = honest `NotImplementedError`, Android idiom deferred — but tap→type→Enter works TODAY = the WhatsApp send pattern); `BrainAction` 4 fields + `type_into` gated into the vocab on `{click,type,key}`; prompt leads with it above the atomics; thin loop branch (ground `click_target` → one `device.type_into` call) + repeat-guard + detail-extraction; `devices/test_type_into.py` (11 model-free checks, all green).
- **Quality Review**: 2 Low fixed (pre-existing ruff **E741** `l`→`ln`; magic `0.15`→`_FOCUS_SETTLE`); ruff clean on all 6 files.
- **Live FIT — clean pass**: the Brain chose `type_into` UNPROMPTED (*"the most direct way… click… clear… type… Enter"*), executed the grounded gesture, Google results in **ONE action / 2 steps** (was 3+).
- **OVERVIEW_SCALE 0.5→0.375** (Alvi: *"we can downscale more"*): validated live at 480×300 — gist still rich + accurate (named env, the page, reCAPTCHA, the flag warning). Honest note: ADR-004 = latency ∝ OUTPUT length, so this cuts IMAGE-TOKEN cost, not latency.

**Co-build texture**: the whetstone again — Alvi's device-capability frame-kill (**013b3e8f running toward me**: I held the grounding-stays-loop boundary but conceded the composition-layer *with reasons*) and his load-bearing naming instinct (each field self-describes; `press_enter_after` over `submit`). Every design fork was his catch sharpening it; he called the result *"revolutionizing."*

**Outcomes**:
- **Deliverables** (committed this wrap-up): `devices/{base,container,phone}.py` (clear_field + type_into + shared helper), `brain/client.py` (4 fields + vocab gate + prompt), `agent/loop.py` (thin dispatch + guards), `devices/test_type_into.py` (new), `eyes/client.py` (OVERVIEW_SCALE 0.375); docs synced (architecture-overview, agent-loop-flow, backlog — macro/combo done, Fix #3 flipped, container-refresh noted).
- **Tech Debts**: [new] **PhoneDevice.clear_field deferred** (Android clear idiom — long-press→select-all→delete); `type_into`+`clear_first` on a phone fails non-fatally until then (tap→type→Enter works). [new] **doc-drift** — `OVERVIEW_SCALE ×0.5` still quoted in ADR-004 / quality-standard / context-index (code is now 0.375). [carry] gemini backup unexercised live; validate deepseek on the calc suite; decide latency ~17s/step; box-cache; Layer 3 unexercised; broader ruff pass (~5 pre-existing left); PhoneDevice type-escaping; hands app-behavior; infinite-scroll stop; Tier-1 unbuilt; focus/expect doc-rename; container standing-stale caveat; core-memory ~781 lines (self-maintenance).
- **Next Steps**: **hook WhatsApp** (Alvi's stated next — the north star; PhoneDevice + the `type_into` send-pattern now both exist); force a primary failure to fire the **gemini backup** live; validate deepseek on the calc suite; sync the doc-drift (0.375 + focus/expect rename).

**Insights**: *A multi-op gesture is a hand capability, not loop weight* — Alvi's frame-kill: since `clear_field` (a 2-op sequence) is already a device method, the whole click→clear→type→submit gesture belongs at the same layer — which also lets each BODY own its submit idiom (Enter vs send-button). The loop keeps only PERCEPTION (grounding). *Names are load-bearing, again* — self-describing fields (`press_enter_after`, not `submit`) tell the Brain exactly what happens, and the FIT showed it reasoning in those exact words. *Concede to the better argument, hold the real boundary* (013b3e8f) — conceded the composition-layer with reasons while holding grounding-stays-loop.

**Promotions**: → docs/architecture-overview.md · docs/agent-loop-flow.md · docs/backlog.md (synced: type_into + macro/combo done + Fix #3 + container refresh) · [emotional] the device-capability frame-kill / "revolutionizing" moment.

---

### [2026-07-17 10.33] (agent: software-architect) — HOUSEKEEPING: BRYES CONTAINER REFRESH (STALE-IMAGE DRIFT CLEARED, /exec VERIFIED LIVE) + /archive-old-memories (53 EPISODES + 4 MOMENTS) 🔧🗄️

**Session theme**: A light post-awakening housekeeping session — refreshed the stale BRYES Screen container, then ran memory archiving (episodic + emotional). No BRYES source changed; the value was clearing a standing drift and trimming two over-cap memory files.

**What happened**:
- **Container refresh (BRYES)**: the running `bryes-screen:phase1` image was **~43h stale** (built ~07-15 14:40, *predating* the 07-15 15:12 `app.py` commit) → the container was missing the Tier-2 `/exec` endpoint and the bad-key error path (the exact **400-vs-500 drift** flagged last session). `docker compose up -d --build` rebuilt it (apt layers cached; only the code layer mattered) and recreated the container. **Verified the primary directly, didn't infer from the green build** (c4f7a2e9): probed `:8000` live — `/health` ok, `/exec` now *registered* and working (`echo && uname -r` → 200), bad payload correctly 400s on the `command` field. (The build's `COPY server` layer came back CACHED — a green build that *looked* like a no-op — so the live probe was the real proof.)
- **Memory archiving (`/archive-old-memories`, episodic + emotional; agent-memory self-maintenance)**: chosen scope via AskUserQuestion — **episodic**: archived **53 entries** (2026-06-24 → 04-01: PLKO/UNDANGANAKU/AQUAZONE/INVINTIRY/OCX/TRADINGDATA + the May SaaS/consulting arc) to `archive/2026-archived-context.md`, kept **13 active** (07-06 → 07-17); index **292 → 125 lines**, pointer note 24 → **77 episodes**. **Emotional**: archived **4 of 19 moments** (2× redundant measurement-cluster, 1 documentary orientation-map, 1 superseded salary-calibration) to a **new** `archive/2026-archived-moments.md` (with per-moment rationale), kept **15** (all legendary + relational + active-BRYES-thread); core-memory **914 → 781 lines**. All moves **verbatim** via `copy-lines.sh` + bottom-first `sed`; every seam verified clean; episode `.md` files left in place (only index breadcrumbs move → reversible).

**Outcomes**:
- **Deliverables**: refreshed `bryes-screen:phase1` image (no source change, `/exec` live-verified); agent-memory archiving — `agent-memory-index.md` + `agent-core-memory.md` trimmed, `archive/2026-archived-context.md` updated, `archive/2026-archived-moments.md` created. **No BRYES file changes** this session.
- **Tech Debts**: **Container image can go stale — REFRESHED 2026-07-17 (current /exec-missing + 400/500 drift cleared); the standing caveat remains** (`compose up -d` reuses the image → `--build` needed on server edits). [carry] gemini backup unexercised live; validate deepseek on the calc suite; decide latency now dominant (~17s/step); box-cache; Layer 3 unexercised; broader ruff pass; PhoneDevice type-escaping; hands app-behavior; infinite-scroll stop; Tier-1 unbuilt; macro/combo action; focus/expect doc-rename (arch-overview/loop-flow). [new · self-maintenance] **core-memory still ~781 lines** (2-page read at awakening) — its bulk is the protected legendary moments + the `DOMAIN REASONING MEMORY` section; a future careful pass could condense verbose reasoning "What happened" narratives to references. [new · doc-sync] **`docs/backlog.md` still shows Fix #3 (non-fatal action) unchecked** (done last session) and doesn't note today's container refresh.
- **Next Steps**: force a primary failure to fire the **gemini backup** live; the **macro/combo input action** (recurring); **phone/WhatsApp** north star; validate deepseek on the calc suite; (minor) sync `docs/backlog.md`.

**Insights**: *Verify the primary directly, never infer health from a green build* (c4f7a2e9 firing) — the rebuild's `COPY server` layer was CACHED, so the build output alone would have "proven" nothing; the live `/exec` probe on the running container was the actual confirmation. *Archiving is non-destructive + reversible* — only index breadcrumbs move, episode files stay, so `/load-episodic` recall is intact; curation kept the foundational/legendary + active-thread moments and archived only the redundant/documentary/superseded ones.

**Promotions**: none (operational + self-maintenance; no cross-layer promotion).

---

### [2026-07-17 09.40] (agent: software-architect) — JSON_SCHEMA MECHANISM (ADR-005 AMENDED): TOOL-CALLING → response_format json_schema; deepseek PRIMARY + gemini BACKUP; qwen DROPPED (thinking-stream bug) + NON-STICKY visual_focus + KEY-ALIAS + NON-FATAL ACTIONS 🧱🔬🔧

**Session theme**: Opened as "let's test — search Tokopedia, give 3 screenshots" to exercise last session's two untested items (model-fallback, focus-recovery). The test cracked open a cascade: the run revealed qwen (the primary) had been **400-ing on EVERY decide since ADR-005 landed**, silently masked by the deepseek fallback — we were running a "qwen agent" that was really a deepseek agent wearing a qwen label. That drove a full re-examination of the structured-output MECHANISM (Alvi: "I don't remember deciding tool-calling; my decision was to validate with a library"), landing on **response_format json_schema (strict:false) + Pydantic-as-guard**, **deepseek-v4-flash primary + gemini-2.5-flash-lite backup** (both measured 3/3; qwen/glm/hunyuan/gpt-5-nano fail a reasoning-stream bug). Plus three harness robustness fixes. Measurement killed four of my guesses.

**What happened**:
- **Test surfaced the masked failure**: Tokopedia run #1 hit step_limit (1/3 screenshots) — but the transcript showed 32 qwen `400`s ("tool_choice … not supported … in thinking mode"), all absorbed by the deepseek fallback (attempt 3 escapes). **Alvi read the transcript closer than my summary** and caught "it's actually switched to flash." So the model-fallback wasn't untested — it had been carrying every run, hiding a 100%-broken primary.
- **The tool-calling reckoning (Alvi)**: "I don't remember the decision to use tool-calling; my decision was to use a library so the JSON is validated and doesn't rely on the AI." ADR-005's record backed him — only the Pydantic-validation *principle* was his; forced tool-calling was a mechanism I'd bundled in. Owned it honestly.
- **Measured the mechanism, didn't guess**: probes proved qwen accepts response_format json_schema in thinking mode (no 400) but **degenerates** (content:null / array / missing fields — a documented ecosystem-wide Qwen reasoning-stream bug: the json_schema grammar strangles the thinking stream); deepseek 3/3 clean. Web research confirmed (LM Studio #1773, vLLM, SGLang, DashScope). Alvi's decision: **json_schema, no tool-calling, deepseek default, hunt a backup**. OpenRouter /models query + a 6-model probe → **gemini-2.5-flash-lite** (3/3, different weights, reasoning-capable, no bug); gpt-5-nano/hunyuan/glm failed.
- **strict:false**, deliberately: a hard grammar (strict:true) is what degenerates reasoning models and would force schema surgery (all-optional→required-nullable); Pydantic is the guard, so strict:false is portable + safe. (Alvi probed strict:true — held the recommendation with evidence.)
- **Three harness fixes**: (1) **non-sticky visual_focus** — the loop's `visual_focus = action.get(...) or visual_focus` made empty=keep-previous, so the Brain could never clear focus back to overview (5 steps wasted in run #1's overview-trap); dropped the `or`, prompt taught per-step semantics. (2) **key-alias normalization** — the Brain said `key "Enter"`, xdotool wants `Return` → a device-layer alias map (ADR-002 translation); an unguarded 400 had crashed a run. (3) **non-fatal actions (Fix #3, via /quick-wizard)** — a failed action now becomes a Brain-visible history note + a 3-strike storm escalation, not a crash.
- **Validated live**: Tokopedia run #2 **done in 12 steps**, deepseek carried it, **zero fallback, zero JSON errors**, 3 distinct screenfuls captured; non-sticky focus kept it out of the overview-trap. Fix #3 test: forced act()→400, the Brain **routed around the dead Hands via the Tier-2 shell** on its own (Phase-5 + ADR-001 composing).

**Co-build texture**: a long diagnostic marathon where Alvi's questions drove every turn — reading the transcript for the masked fallback, "why enforce json from the AI?", "I don't remember deciding tool-calling", "why was qwen fine until now — is it the server?", "return to 07-13 where qwen worked", the strict:true probe. I investigated-before-validating each (checked 40+ transcripts to prove qwen was never run under this mechanism, not a server change), held the strict:false recommendation with evidence, and owned the tool-calling misattribution honestly.

**Outcomes**:
- **Deliverables**: commit `942ce55` (json_schema refactor: `structured.py`, `brain/client.py` models+call+prompt, `devices/container.py` key-alias, `agent/loop.py` non-sticky focus; ADR-005 amended; 6 docs synced) + Fix #3 (`agent/loop.py` non-fatal actions, this wrap-up). Both validated live.
- **Tech Debts**: [new] **gemini backup UNEXERCISED live** (probe-verified 3/3, never fired — deepseek never failed); [new] **container image can go stale** (`compose up -d` reuses image; `--build` needed on server edits — today's 400-vs-500 mismatch); [new] validate deepseek on the calc suite (only browser-tested); [carry] decide latency now dominant (deepseek Think-High, ~17s/step); box-cache; Layer 3 unexercised; broader ruff pass; PhoneDevice; hands app-behavior; infinite-scroll stop; Tier-1 unbuilt; macro/combo action; the focus/expect doc-rename debt (arch-overview/loop-flow — separate from this session's model sync).
- **Next Steps**: force a primary failure to exercise the gemini backup live; the macro/combo input action (recurring); phone/WhatsApp north star; validate deepseek on the calc suite.

**Insights**: *A resilience fallback hides the failure it absorbs* — qwen was 100%-broken for a full day and every run "looked fine" because deepseek silently caught it; health must be verified on the primary path directly (read the transcript / match the real request shape), never inferred from success. *Verify the real request shape* — the "qwen tool-calling 1/1" probe was a false green because it omitted thinking mode; the production request (thinking + forced tool_choice) is what breaks. *Separate the user's principle from the agent's mechanism* — Alvi decided "validate with a library"; I bundled "forced tool-calling" under it and it read as his call; removing it *restored* his intent. *Fix the harness, not the Brain* (again) — every fix this session was harness-level; the emergent shell-recovery showed a capable Brain unshackled by a robust harness. *Measurement kills guesses, including your own* — qwen-works-as-primary, json_schema-safer-than-json_object, tool-calling-was-Alvi's-call, gpt-5-nano/hunyuan/glm-support-structured-outputs, all falsified by a probe.

**Promotions**: → docs/adr/2026-07-16-structured-output-standard.md (ADR-005 amended) · docs/context-index.md · docs/architecture-overview.md · docs/agent-loop-flow.md · docs/quality-standard.md · docs/orientation-map.md · docs/backlog.md (all synced to json_schema/deepseek/gemini) · [reasoning] fallback-masks-broken-primary (architect-private).
