...
| [quality-standard.md](quality-standard.md) | The project quality standard (9 dimensions, discovered from code). Dimension 9's headline: **formats are enforced by tools, not the AI** — LLM JSON via Pydantic + forced tool-calling + validation ([ADR-005](adr/2026-07-16-structured-output-standard.md)). Used by `/analyze-code-quality`. | quality, standard, structured-output, conventions, ruff |
...
- **Brain ([ADR-005](adr/2026-07-16-structured-output-standard.md)):** primary `qwen/qwen3.6-flash`, backup `deepseek/deepseek-v4-flash` (decide's LAST attempt escapes to it if the primary fails its tries). Output is a **Pydantic model → forced tool-call → OUR validation** (`structured.py`) — NEVER `json_object` free-text; validity never depends on the provider. Swappable via `run(brain_model=...)`.
- **Structured output & focus-failure:** any format-bearing LLM output goes through `structured.py` (Pydantic + tool-calling). The Eyes report honestly: `box()` answers `NOT_FOUND` instead of fabricating coords → `describe` gives `VISUAL_FOCUS FAILED` + an overview so the Brain re-orients. The Brain aims `visual_focus` at where an action's EFFECT shows (the display), not the control it pressed.
